...
with open(sys.argv[1]) as r:
	for line in r:
		read_name_1 = line.strip()
		read_name = "_".join(read_name_1.split(" "))
		read = next(r).strip()
		qual_name = next(r).strip()
		qual = next(r).strip()
		current_bc = read[4:9]
		current_umi = read[0:4]+read[9:11]
		if read[4:9] in sample_index.keys():
			...
			exec("write_none.write('%s_%s')" % (read_name,"umi:"+current_umi))
			write_none.write('\n')
			exec("write_none.write('%s')" % (read[11:]))
			write_none.write('\n')
			exec("write_none.write('%s')" % (qual_name))
			write_none.write('\n')
